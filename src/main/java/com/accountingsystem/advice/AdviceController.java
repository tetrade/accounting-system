...
public class AdviceController {
    ...
    public ResponseEntity<AppError> catchNoSuchRowException(NoSuchRowException ex) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new AppError(ex.getMessage()));
    }
    ...
    public ResponseEntity<AppError> catchSQLIntegrityConstraintViolationException(SQLIntegrityConstraintViolationException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(new AppError(ex.getMessage()));
    }
    ...
    public ResponseEntity<Violation> catchInvalidFormatException(InvalidFormatException ex) {
        ...
        if (ex.getValue().toString().equals("")) {
            v.setMessage("не может быть пустым");
        } else if (ex.getTargetType().isEnum()) {
            v.setMessage("возможные значения: " + ex.getOriginalMessage().split("(\\[)|(\\])")[1]);
        } else if (ex.getTargetType().equals(BigDecimal.class)) {
            v.setMessage("неправильный числовой формат");
        } else if (ex.getTargetType().equals(LocalDate.class)) {
            v.setMessage("неправильный формат даты, должен быть dd.MM.yyyy");
        } else {
            v.setMessage(ex.getLocalizedMessage());
        }
